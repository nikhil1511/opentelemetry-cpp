...
TEST(BaggageTest, ValidateHeaderParsing)
{
    ...
    auto invalid_total_size_header = header_with_custom_size(Baggage::kMaxKeyValueSize, num_pairs_with_max_size + 1);
    auto valid_header_with_max_size_pairs = header_with_custom_size(Baggage::kMaxKeyValueSize, num_pairs_with_max_size);
...
TEST(BaggageTest, BaggageSet)
{
    ...
    auto baggage_new_2 = baggage_new->Set("k3", "v3_1"); // key should be updated with the latest value
    ...
    auto baggage2_new_2 = baggage2_new->Set("key0", "0");  // updating on max list with size threshold should work
    ...
    auto baggage3_new_2      = baggage3->Set("k3", invalid_max_size_value);  // adding pair with length greater than threshold should be discarded.
    EXPECT_EQ(baggage2_new_2->Get("k3"), "");

    auto baggage3_new_3  = baggage3->Set("k2", invalid_max_size_value);  // adding pair with length greater than threshold should be discarded.
                                                                         // old baggage should be returned.
    ...
    auto baggage4_new = baggage4->Set("k", std::string((int)Baggage::kMaxKeyValueSize - 1, 'a')); // valid pair which increases size of baggage beyond
