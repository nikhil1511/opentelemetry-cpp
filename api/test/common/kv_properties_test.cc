...
TEST(KVStringIter, SinglePair)
{
  ...
  auto kv_pair = itr.next();
  EXPECT_EQ(kv_pair.status, KeyValueStringIterator::Status::VALID);
  EXPECT_EQ(kv_pair.key, "k1");
  EXPECT_EQ(kv_pair.value, "v1");
  itr.next();
  EXPECT_EQ(kv_pair.status, KeyValueStringIterator::Status::END);
}
*/
